 ...
 */
public class MinTwoHeapComparable<E extends Comparable<E>> extends PriorityWorkList<E> {

    private final static int K_VALUE = 4;

    /* Do not change the name of this field; the tests rely on it to work correctly. */
    private E[] data;
    private int size;

    public MinTwoHeapComparable() {
    ...
    private int percolateUp(int nodePosition, E value) {
        while (nodePosition > 0 && value.compareTo(data[(nodePosition - 1) / K_VALUE]) < 0) {
            data[nodePosition] = data[(nodePosition - 1) / K_VALUE];
            nodePosition = (nodePosition - 1) / K_VALUE;
        }
        return nodePosition;
    }

    private int percolateDown(int nodePosition, E value) {
        while (nodePosition * K_VALUE + 1 < size) {

            //Find the smallest child
            int firstChild = nodePosition * K_VALUE + 1;
