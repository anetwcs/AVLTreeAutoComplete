...
//  private static final String CORPUS = "spoken.corpus";
    // private static final String CORPUS = "me.txt";


    // When N = 10, we could see each data type took the following time for initialization.
    // For word searches, all data types showed 0ms because there are few cases.
    // For the following reasons, we decided to use ChainingHashTable(MoveToFrontList) for the outer map and AVLTree for the inner map.
    //
    // 1) Such a combination showed the best initialization time.
    // 2) ChainingHashTable::MoveToFrontList was better than ChainingHashTable::AVLTree in average cases based on our write-up experiment.
    // 3) Since we didn't want to waste memory for the inner map as long as it shows similar find performance,
    //    there was no reason to use ChainingHashTable instead of AVLTree for the inner map even though it might be a little faster in some situations.

    private static final Supplier<Dictionary<NGram, Dictionary<AlphabeticString, Integer>>> NEW_OUTER = () -> new ChainingHashTable(MoveToFrontList::new);
    // private static final Supplier<Dictionary<NGram, Dictionary<AlphabeticString, Integer>>> NEW_OUTER = () -> new ChainingHashTable(AVLTree::new);
    // private static final Supplier<Dictionary<NGram, Dictionary<AlphabeticString, Integer>>> NEW_OUTER = NGramTester.trieConstructor(NGram.class);
    // private static final Supplier<Dictionary<NGram, Dictionary<AlphabeticString, Integer>>> NEW_OUTER = () -> new BinarySearchTree();
    // private static final Supplier<Dictionary<NGram, Dictionary<AlphabeticString, Integer>>> NEW_OUTER = () -> new ChainingHashTable();
    // private static final Supplier<Dictionary<NGram, Dictionary<AlphabeticString, Integer>>> NEW_OUTER = () -> new AVLTree();
    // private static final Supplier<Dictionary<NGram, Dictionary<AlphabeticString, Integer>>> NEW_OUTER = () -> new MoveToFrontList();

    private static final Supplier<Dictionary<AlphabeticString, Integer>> NEW_INNER = () -> new AVLTree<>();
    // private static final Supplier<Dictionary<AlphabeticString, Integer>> NEW_INNER = NGramTester.trieConstructor(AlphabeticString.class);
    // private static final Supplier<Dictionary<AlphabeticString, Integer>> NEW_INNER = () -> new ChainingHashTable(MoveToFrontList::new);
    // private static final Supplier<Dictionary<AlphabeticString, Integer>> NEW_INNER = () -> new BinarySearchTree();
    // private static final Supplier<Dictionary<AlphabeticString, Integer>> NEW_INNER = () -> new MoveToFrontList();
    ...
    static class MarkovLoader implements Runnable {
        ...
        public void run() {
            ...
            try {
                uMessage.markov[this.i] = new WordSuggestor(uMessage.CORPUS, N - this.i, 4, uMessage.NEW_OUTER, uMessage.NEW_INNER);
                this.window.update();
            } catch (IOException e) {
                e.printStackTrace();
            }
